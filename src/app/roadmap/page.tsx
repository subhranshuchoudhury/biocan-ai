...
export default function RoadMap() {
    ...
    const getSuggestedJobs = async (mbtyType: string) => {
        try {
            const options = {
                method: 'POST',
            ...
            setLoading(true)
            const response = await fetch(`https://biocan-backend.onrender.com/get_recommended_jobs?mbti=${mbtyType}`, options)
            ...
            <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full px-4 py-2">
                {
                    <div className="flex-1 bg-[url(/assets/chat-bg.svg)] bg-cover bg-no-repeat shrink-0 bg-center rounded-lg shadow-md p-4 overflow-y-auto mb-2 max-h-[calc(100vh-200px)]">
                        <div>
                            ...
                            {
                                Loading && <div className="mt-4 text-left">
                                    <div className="my-2 p-3 rounded-lg hover:cursor-pointer bg-[#F3F3F3] text-black rounded-bl-none max-w-[80%] sm:max-w-[60%] flex items-center justify-between">
                            ...
                            <div className="mt-4 text-left">
                                {
                                    JobsData.job_roles.map((job, index) => {
                                        return (
                                            <div key={index} onClick={() => handleOnClickJob(job)} className="my-2 p-3 rounded-lg hover:cursor-pointer bg-[#F3F3F3] text-black rounded-bl-none max-w-[80%] sm:max-w-[60%] flex items-center justify-between">
                                                <span>{job}</span>
